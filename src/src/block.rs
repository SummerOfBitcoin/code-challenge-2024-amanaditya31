...
pub(crate) fn block_maker() {
    ...
    let coinbase_txn = create_coinbase(&merkle_wtxid, &transactions.2);
...
fn mine_header(target: &str, header: String) -> String {
    ...
    let header_byte_ = hex::decode(header).unwrap();
    
    loop {
        
        let mut header_byte = header_byte_.clone();
        header_byte.extend(nonce.to_le_bytes());

        let mut hash_bytes = hash256(&header_byte);
        hash_bytes.reverse();
        
        if hash_bytes < target_bytes {
            println!("No block");
            println!("Nonce = {}", nonce);
            return hex::encode(&header_byte);
        }
        nonce = nonce + 1;
    }
}

fn create_txid_wtxid(txns: &[String], wtxns: &[String]) -> (Vec<String>, Vec<String>) {
...
fn transaction_selector(txns: Vec<String>) -> (Vec<String>, Vec<String>, usize) {
    ...
    for transaction in txns {
        ...
        if !check_p2wpkh(&tx) || !validation::validate_segwit(&tx) {
            continue;
        }
...
fn create_coinbase(merkle_root: &String, txn_fees: &usize) -> (String, String) {
...
fn create_merkle_root(transactions: &Vec<String>) -> String {
    ...
    for i in (0..transactions.len()).step_by(2) {
        ...
        if i < transactions.len() - 1 { txn2 = &transactions[i + 1]; } 
        else {txn2 = txn1;}
...
fn read_trasactions() -> Vec<String> {
    ...
    for transaction in directory {
        ...
        if transaction.path().is_file() {
            ...
            file.read_to_string(&mut tx_data).expect("Error reading file");
